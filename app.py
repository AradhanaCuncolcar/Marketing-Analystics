...
# Page Configuration
# ==========================================
st.set_page_config(page_title="NovaMart Dashboard", layout="wide", page_icon="🛍️")
...
def apply_custom_css():
    st.markdown("""
        <style>
        /* Smooth, modern font styling */
        @import url('https://fonts.googleapis.com/css2?family=Inter:wght@400;500;600&display=swap');
        html, body, [class*="css"]  {
            font-family: 'Inter', sans-serif;
        }
        
        /* Style the metric cards to look like modern dashboard widgets */
        ...
        /* Subtle hover effect for metrics */
        [data-testid="stMetric"]:hover {
            box-shadow: 0 4px 6px -1px rgb(0 0 0 / 0.1), 0 2px 4px -2px rgb(0 0 0 / 0.1);
            transform: translateY(-2px);
        }

        /* Clean up standard container borders */
        ...
        /* Style headers for a cleaner hierarchy */
        h1, h2, h3 {
            color: #1e293b;
...
def style_plotly_fig(fig):
    """Applies a clean, modern aesthetic to Plotly figures."""
    fig.update_layout(
    ...
    # Clean up gridlines for a minimalist look
...
def plot_revenue_trend(df, agg_toggle):
    ...
    return style_plotly_fig(fig), df_grouped
...
def plot_satisfaction_violin(df, split_by_channel):
    ...
    fig = px.violin(df, x="nps_category", y="satisfaction_score", color=color_col, box=True)
    return style_plotly_fig(fig)
...
def plot_income_ltv_scatter(df, trend):
    ...
    fig = px.scatter(df, x="income", y="lifetime_value", color="customer_segment", trendline=trendline_type, hover_data=['age', 'city_tier'])
    return style_plotly_fig(fig)

def plot_ctr_cvr_bubble(df):
    fig = px.scatter(df, x="ctr", y="conversion_rate", size="spend", color="channel", hover_name="channel", size_max=60, title="Bubble size = Total Spend")
    return style_plotly_fig(fig)
...
def plot_geo_store_bubbles(df):
    fig = px.scatter_mapbox(df, lat="latitude", lon="longitude", hover_name="state", size="store_count", color="customer_satisfaction", color_continuous_scale=px.colors.cyclical.IceFire, size_max=25, zoom=3, mapbox_style="carto-positron")
    return style_plotly_fig(fig)
...
def plot_sankey_journey(df):
    ...
    fig = go.Figure(data=[go.Sankey(node=dict(pad=15, thickness=20, line=dict(color="black", width=0.5), label=nodes), link=dict(source=source, target=target, value=value))])
    return style_plotly_fig(fig)
...
# Main App Execution
# ==========================================
st.sidebar.title("🛍️ NovaMart Analytics")
st.sidebar.markdown("---")

# Cleaner navigation styling using label_visibility
page = st.sidebar.radio(
    "Navigation", 
    ["Executive Overview", "Campaign Analytics", "Customer Insights", "Product Performance", "Geographic Analysis", "Attribution & Funnel", "ML Model Evaluation"],
