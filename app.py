...
# Page Configuration
# ==========================================
st.set_page_config(page_title="NovaMart Dashboard", layout="wide", page_icon="將")

# ==========================================
# Helper Functions for Formatting
# ==========================================
def format_large_number(num):
    """Formats large numbers into K, M, B for cleaner display in metric boxes."""
    try:
        num = float(num)
...
if page == "Executive Overview":
    ...
    with col1:
        with st.container(border=True):
            st.metric("Total Revenue", f"${format_large_number(total_revenue)}")
    with col2:
        with st.container(border=True):
            st.metric("Total Conversions", f"{format_large_number(total_conversions)}")
    with col3:
        with st.container(border=True):
            st.metric("Average ROAS", f"{avg_roas:.2f}x")
    with col4:
        with st.container(border=True):
            st.metric("Total Customers", f"{format_large_number(total_customers)}")
